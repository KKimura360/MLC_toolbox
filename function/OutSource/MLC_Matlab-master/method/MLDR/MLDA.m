...
% Step 2. Sovle OPLS using different implemetations. The default choice is solving the eigenvalue problem.
    C=squareform(1-pdist(Y,'cosine')); % L x L correlation matrix
    Z=Y'*C;  % N x L matrix 
    %%normalization using (15) or (16) in the paper,  
   % normZ=sum(Y);  %% (15) following the paper, we use (15)
    normZ=sum(Z,2); %% (16)
    Z= diag(normZ.^(-1)) * Z; %% normalize each row column of Z
    Y=Z';
    % After this use Z instead Y;      
