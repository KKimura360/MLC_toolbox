...
void computeknn(double *Z, double *Zt, double *neighIdx, double *neighVal, long long numneighbors, long long n, long long k, long long nt, int numThreads)
{	
	
	double *normZ = (double *)malloc(sizeof(double)*n);
	...
	{	
		...
		{
			normZ[trainIter] += (Z[trainIter*k + dimIter]*Z[trainIter*k + dimIter]);
		}
	}
    #pragma omp parallel for num_threads(numThreads)
...
{	
	...
	computeknn(Z, Zt, neighIdx, neighVal, numneighbors, n, k, nt, numThreads);
}