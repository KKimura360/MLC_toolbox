function [bestK, bestT, bestL, kmatrix, errlengthmatrix, bestB, bestX] = smdl4bmf(A, T, varargin)
% SMDL4BMF - computes BMF for sparse matrices using ASSO and MDL
% 
% Usage
%  k = SMDL4BMF(A, T) 
...
%
%    'processes'            Number of processes to launch (default = 2)
...
if ~issparse(A)
    warning('SMDL4BMF:sparseInput', 'Input matrix is not sparse, consider using MDL4BMF instead');
    A = sparse(A);
end
...
% sanity check
if maxK > min(n,m), 
  warning('SMDL4BMF:largeK', 'Setting maximum k to min(n, m) = %i', min(n,m));
  maxK = min(n,m);
end

% Temp check
if processes < 2
    warning('SMDL4BMF:nonParallel', 'Non-parallel running is not optimized');
end
...
if nargout > 5,
    if strcmp(errorMeasure, 'all')
        warning('SMDL4BMF:returnValues', 'Only returning ''k'', ''t'', ''L'', ''M'', and ''ME'' are currently implemented if using ''all'' as an error measure');
    else
        returnBandX = true;
    end
end
...
if strcmp(errorMeasure, 'all')
    ...
    if isfinite(maxItersSinceBetter)
      warning('SMDL4BMF:maxItersSinceBetter', 'Using maxItersSinceBetter is only supported with one error measure');
    end
else
...
Corig = full(A*A'); % Association confidences
...
for titer=1:length(T)
    ...
    for k=1:maxK,
        ...
        %% corresponding row to X. avail_id is index to C.
	    %% If processes > 1, use parallel version
	    %if processes > 1,
        [avail_id, r] = select_best_column_sparse_par(A, ...
                                                        C, ...
                                                        available_cols, ...
                                                        mask, ...
                                                        processes);
        B(:,k) = full(double(C(:,available_cols(avail_id))));
        X(k,:) = r;
        %% DEBUG
        %disp('B')
        %disp(B)
